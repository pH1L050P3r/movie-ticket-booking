...
public final class ClusterListener
  extends AbstractBehavior<ClusterListener.Event> {
  ...
  private ClusterListener(ActorContext<Event> context) {
    ...
    ActorRef<ClusterEvent.MemberEvent> memberEventAdapter = context.messageAdapter(
    ...
    cluster
      .subscriptions()
      .tell(
        Subscribe.create(memberEventAdapter, ClusterEvent.MemberEvent.class)
      );

    ActorRef<ClusterEvent.ReachabilityEvent> reachabilityAdapter = context.messageAdapter(
      ClusterEvent.ReachabilityEvent.class,
      ReachabilityChange::new
    );
    cluster
      .subscriptions()
      .tell(
    ...
    if (event.reachabilityEvent instanceof ClusterEvent.UnreachableMember) {
      getContext()
        ...
        .info(
          ...
          event.reachabilityEvent.member()
        );
    } else if (
      event.reachabilityEvent instanceof ClusterEvent.ReachableMember
    ) {
      getContext()
        .getLog()
  ...
  private Behavior<Event> onMemberChange(MemberChange event) {
    if (event.memberEvent instanceof ClusterEvent.MemberUp) {
      getContext()
        .getLog()
        .info("Member is up: {}", event.memberEvent.member());
